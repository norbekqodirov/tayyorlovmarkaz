...
import { FileText, Plus, Edit3, Trash2, BarChart3, Eye, CheckCircle2, Clock, Users, Send, X, Award, TrendingUp } from 'lucide-react';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Cell } from 'recharts';
import { toLetterGrade, getGradeBgColor, getGradeColor } from '../../../utils/grading';
...
export default function CrmTests() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Results */}
      <Modal isOpen={resultsOpen} onClose={() => setResultsOpen(false)} title="Test Natijalari" width="2xl">
        {results && <TestResultsView results={results} />}
      </Modal>
    </div>
  );
}
...
// ─── Test Results View ────────────────────────────────────────────────────────
function TestResultsView({ results }: { results: any }) {
  ...
  const distribution = useMemo(() => {
    ...
    submissions.forEach((s: any) => {
      ...
      const family = letter.charAt(0);
      dist[family] = (dist[family] || 0) + 1;
    });
        ...
        <div className="bg-zinc-50 dark:bg-zinc-800/30 rounded-2xl p-4 border border-zinc-100 dark:border-zinc-800">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <p className="text-xs font-black text-slate-900 dark:text-white">Baholar Taqsimoti</p>
            </div>
            <span className="text-[10px] font-bold text-zinc-400">Xalqaro standart (A–F)</span>
          </div>
          <ResponsiveContainer width="100%" height={150}>
            <BarChart data={distribution}>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(0,0,0,0.06)" vertical={false} />
              <XAxis dataKey="letter" fontSize={12} fontWeight={700} stroke="#94a3b8" />
        ...
        <table className="w-full text-sm">
          <thead className="bg-zinc-50 dark:bg-zinc-800/50 sticky top-0 z-10">
            <tr>
              ...
              <th className="px-4 py-2.5 text-center text-xs font-black text-zinc-400 uppercase tracking-widest">Ball</th>
              <th className="px-4 py-2.5 text-center text-xs font-black text-zinc-400 uppercase tracking-widest">Baho</th>
              <th className="px-4 py-2.5 text-center text-xs font-black text-zinc-400 uppercase tracking-widest">Holat</th>
              <th className="px-4 py-2.5 text-right text-xs font-black text-zinc-400 uppercase tracking-widest">Vaqt</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
            {submissions.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-3 py-12 text-center text-zinc-400">Hali natijalar yo'q</td>
              </tr>
            ) : (
              submissions.map((s: any) => {
                const score = s.score || 0;
                const letter = s.status === 'graded' ? toLetterGrade(score) : null;
                return (
                  <tr key={s.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors">
                    <td className="px-4 py-2.5 font-bold text-slate-900 dark:text-white">{s.student?.name || 'Anonim'}</td>
                    <td className="px-4 py-2.5 text-center">
                      <span className={`font-black ${getGradeColor(letter || 'F')}`}>{Math.round(score)}%</span>
                    </td>
                    <td className="px-4 py-2.5 text-center">
                      {letter ? (
                        <span className={`inline-block px-2.5 py-1 rounded-lg border text-xs font-black ${getGradeBgColor(letter)}`}>
                          {letter}
                        </span>
                    ...
                    <td className="px-4 py-2.5 text-center">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest ${
                        s.status === 'graded' ? 'bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-400' :
                        s.status === 'submitted' ? 'bg-amber-100 dark:bg-amber-500/15 text-amber-700 dark:text-amber-400' :
                        'bg-zinc-100 dark:bg-zinc-800 text-zinc-600'
                      }`}>
                        {s.status === 'graded' ? 'Baholangan' : s.status === 'submitted' ? 'Tekshirilmoqda' : 'Davom etmoqda'}
                      </span>
                    </td>
                    <td className="px-4 py-2.5 text-right text-xs text-zinc-500 font-medium">
                      {s.submittedAt ? new Date(s.submittedAt).toLocaleString('uz-UZ', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }) : '—'}
                    </td>
                  </tr>
                );
              })
